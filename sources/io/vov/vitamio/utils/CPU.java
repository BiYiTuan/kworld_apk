...
public class CPU {
    ...
    public static int getFeature() {
        ...
        if (cpuinfo.isEmpty()) {
            ...
            try {
                BufferedReader bis = new BufferedReader(new FileReader(new File("/proc/cpuinfo")));
                while (true) {
                    try {
                ...
                try {
                    ...
                    if (bufferedReader != null) {
                        try {
                            ...
                            Log.m32e("getCPUFeature", e32);
                        }
                    }
                    if (!cpuinfo.isEmpty()) {
                        for (String key : cpuinfo.keySet()) {
                            Log.m31d("%s:%s", key, cpuinfo.get(key));
                        }
                        hasARMv6 = false;
                        hasARMv7 = false;
                        ...
                        if (TextUtils.isEmpty(val)) {
                            ...
                            try {
                                ...
                                Log.m31d("CPU architecture: %s", Integer.valueOf(i));
                                if (i >= 7) {
                                    hasARMv6 = true;
                                    hasARMv7 = true;
                                } else if (i >= 6) {
        ...
        if (cpuinfo.isEmpty()) {
            for (String key2 : cpuinfo.keySet()) {
                Log.m31d("%s:%s", key2, cpuinfo.get(key2));
            }
            hasARMv6 = false;
            hasARMv7 = false;
            val = (String) cpuinfo.get("CPU architecture");
    ...
    private static int getCachedFeature() {
        ...
        Log.m31d("GET CPU FATURE: %s", cachedFeatureString);
