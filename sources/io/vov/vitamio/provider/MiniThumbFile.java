...
public class MiniThumbFile {
    ...
    private String randomAccessFilePath(int version) {
        return (Environment.getExternalStorageDirectory().toString() + InternalZipConstants.ZIP_FILE_SEPARATOR + Thumbnails.THUMBNAILS_DIRECTORY) + "/.thumbdata" + version + "-" + this.mUri.hashCode();
    }
    ...
    private RandomAccessFile miniThumbDataFile() {
        if (this.mMiniThumbFile == null) {
            ...
            if (!(directory.isDirectory() || directory.mkdirs())) {
                Log.m33e("Unable to create .thumbnails directory %s", directory.toString());
            }
            File f = new File(path);
            try {
                this.mMiniThumbFile = new RandomAccessFile(f, InternalZipConstants.WRITE_MODE);
            } catch (IOException e) {
                try {
                    this.mMiniThumbFile = new RandomAccessFile(f, InternalZipConstants.READ_MODE);
    ...
    protected synchronized long getMagic(long id) {
        ...
        if (miniThumbDataFile() != null) {
            ...
            try {
                ...
                Log.m32e("Got exception checking file magic: ", ex);
                if (lock != null) {
                    try {
                        lock.release();
                ...
                Log.m33e("Got exception when reading magic, id = %d, disk full or mount read-only? %s", Long.valueOf(id), ex2.getClass().toString());
                if (lock != null) {
                    try {
                        lock.release();
        ...
        if (miniThumbDataFile() != null) {
            ...
            if (data != null) {
                try {
                    ...
                    Log.m33e("couldn't save mini thumbnail data for %d; %s", Long.valueOf(id), ex.getMessage());
                    throw ex;
                } catch (RuntimeException ex2) {
                    Log.m33e("couldn't save mini thumbnail data for %d, disk full or mount read-only? %s", Long.valueOf(id), ex2.getClass().toString());
                    if (lock != null) {
                        try {
                            lock.release();
                        } catch (IOException e2) {
    ...
    protected synchronized byte[] getMiniThumbFromFile(long id, byte[] data) {
        if (miniThumbDataFile() == null) {
            ...
            try {
                ...
                Log.m33e("got exception when reading thumbnail id = %d, exception: %s", Long.valueOf(id), ex.getMessage());
                if (lock != null) {
                    try {
                        lock.release();
                    } catch (IOException e3) {
                ...
                Log.m33e("Got exception when reading thumbnail, id = %d, disk full or mount read-only? %s", Long.valueOf(id), ex2.getClass().toString());
                if (lock != null) {
                    try {
                        lock.release();
                    } catch (IOException e4) {
