...
class ResponseCachingPolicy {
    ...
    private static final Set<Integer> cacheableStatuses = new HashSet(Arrays.asList(new Integer[]{Integer.valueOf(200), Integer.valueOf(HttpStatus.SC_NON_AUTHORITATIVE_INFORMATION), Integer.valueOf(300), Integer.valueOf(HttpStatus.SC_MOVED_PERMANENTLY), Integer.valueOf(HttpStatus.SC_GONE)}));
    ...
    private boolean unknownStatusCode(int status) {
        ...
        if (status >= 300 && status <= HttpStatus.SC_TEMPORARY_REDIRECT) {
            return false;
        }
        if (status >= 400 && status <= HttpStatus.SC_EXPECTATION_FAILED) {
            return false;
        }
    ...
    private boolean from1_0Origin(HttpResponse response) {
        ...
        if (via != null) {
            ...
            if (0 < elements.length) {
                ...
                if (proto.contains(InternalZipConstants.ZIP_FILE_SEPARATOR)) {
