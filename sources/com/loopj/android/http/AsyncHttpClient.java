...
public class AsyncHttpClient {
    ...
    class C09171 implements HttpRequestInterceptor {
        C09171() {
        }
        ...
        public void process(HttpRequest request, HttpContext context) {
            ...
            for (String header : AsyncHttpClient.this.clientHeaderMap.keySet()) {
                if (request.containsHeader(header)) {
                    ...
                    AsyncHttpClient.log.mo1728d(AsyncHttpClient.LOG_TAG, String.format("Headers were overwritten! (%s | %s) overwrites (%s | %s)", new Object[]{header, AsyncHttpClient.this.clientHeaderMap.get(header), overwritten.getName(), overwritten.getValue()}));
    ...
    class C09182 implements HttpResponseInterceptor {
        C09182() {
        }
    ...
    class C09193 implements HttpRequestInterceptor {
        C09193() {
        }
    ...
    public AsyncHttpClient(SchemeRegistry schemeRegistry) {
        ...
        this.httpClient.addRequestInterceptor(new C09171());
        this.httpClient.addResponseInterceptor(new C09182());
        this.httpClient.addRequestInterceptor(new C09193(), 0);
    ...
    private static SchemeRegistry getDefaultSchemeRegistry(boolean fixNoHttpResponseException, int httpPort, int httpsPort) {
        ...
        if (fixNoHttpResponseException) {
            log.mo1728d(LOG_TAG, "Beware! Using the fix is insecure, as it doesn't verify SSL certificates.");
        }
        if (httpPort < 1) {
            httpPort = 80;
            log.mo1728d(LOG_TAG, "Invalid HTTP port number specified, defaulting to 80");
        }
        if (httpsPort < 1) {
            httpsPort = 443;
            log.mo1728d(LOG_TAG, "Invalid HTTPS port number specified, defaulting to 443");
        }
        if (fixNoHttpResponseException) {
    ...
    public static String getUrlWithQueryString(boolean shouldEncodeUrl, String url, RequestParams params) {
        ...
        if (shouldEncodeUrl) {
            try {
                ...
                log.mo1731e(LOG_TAG, "getUrlWithQueryString encoding URL", ex);
            }
        }
        if (params != null) {
    ...
    public static void silentCloseInputStream(InputStream is) {
        if (is != null) {
            try {
                ...
                log.mo1742w(LOG_TAG, "Cannot close input stream", e);
            }
        }
    }
    ...
    public static void silentCloseOutputStream(OutputStream os) {
        if (os != null) {
            try {
                ...
                log.mo1742w(LOG_TAG, "Cannot close output stream", e);
            }
        }
    }
    ...
    public static void endEntityViaReflection(HttpEntity entity) {
        if (entity instanceof HttpEntityWrapper) {
            ...
            try {
                ...
                log.mo1731e(LOG_TAG, "wrappedEntity consume", t);
            }
        }
    }
    ...
    public void setCredentials(AuthScope authScope, Credentials credentials) {
        if (credentials == null) {
            log.mo1728d(LOG_TAG, "Provided credentials are null, not setting");
            return;
        }
    ...
    public void cancelRequests(Context context, final boolean mayInterruptIfRunning) {
        if (context == null) {
            log.mo1730e(LOG_TAG, "Passed null Context to cancelRequests");
            return;
        }
    ...
    public void cancelRequestsByTAG(Object TAG, boolean mayInterruptIfRunning) {
        if (TAG == null) {
            log.mo1728d(LOG_TAG, "cancelRequestsByTAG, passed TAG is null, cannot proceed");
            return;
        }
        for (List<RequestHandle> requestList : this.requestMap.values()) {
            if (requestList != null) {
        ...
        if (uriRequest == null) {
            ...
            if (contentType != null) {
                if ((uriRequest instanceof HttpEntityEnclosingRequestBase) && ((HttpEntityEnclosingRequestBase) uriRequest).getEntity() != null && uriRequest.containsHeader("Content-Type")) {
                    log.mo1741w(LOG_TAG, "Passed contentType will be ignored because HttpEntity sets content type");
                } else {
