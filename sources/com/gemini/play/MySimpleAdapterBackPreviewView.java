...
public class MySimpleAdapterBackPreviewView extends SimpleAdapter {
    private Context _this;
    private int current_index = 0;
    private float rate;
    private Typeface typeFace;

    public MySimpleAdapterBackPreviewView(Context context, List<? extends Map<String, ?>> data, int resource, String[] from, int[] to) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (MGplayer.getDensityDpi() <= ThumbnailUtils.TARGET_SIZE_MINI_THUMBNAIL_HEIGHT) {
            layoutParams.width = (int) (this.rate * 6.0f);
            layoutParams.height = (int) (this.rate * 20.0f);
        } else {
            layoutParams.width = (int) (this.rate * 6.0f);
            layoutParams.height = (int) ((this.rate * 20.0f) * (1.0f + (((float) (MGplayer.getDensityDpi() - 320)) / 160.0f)));
        }
