...
final class SubtitleParserHelper implements Callback {
    ...
    private void handleFormat(MediaFormat format) {
        this.subtitlesAreRelative = format.subsampleOffsetUs == MediaFormat.OFFSET_SAMPLE_RELATIVE;
    ...
    private void handleSample(long sampleTimeUs, SampleHolder holder) {
        ...
        try {
            parsedSubtitle = this.parser.parse(new ByteArrayInputStream(holder.data.array(), 0, holder.size));
        } catch (IOException e) {
            error = e;
