...
public class DashChunkSource implements ChunkSource, Output {
    ...
    private static final class RepresentationHolder {
        ...
        public RepresentationHolder(long periodStartTimeUs, long periodDurationUs, Representation representation) {
            ...
            if (DashChunkSource.mimeTypeIsRawText(mimeType)) {
    ...
    public DashChunkSource(MediaPresentationDescription manifest, DashTrackSelector trackSelector, DataSource dataSource, FormatEvaluator adaptiveFormatEvaluator) {
        this(null, manifest, trackSelector, dataSource, adaptiveFormatEvaluator, new SystemClock(), 0, 0, false, null, null);
    }

    public DashChunkSource(ManifestFetcher<MediaPresentationDescription> manifestFetcher, DashTrackSelector trackSelector, DataSource dataSource, FormatEvaluator adaptiveFormatEvaluator, long liveEdgeLatencyMs, long elapsedRealtimeOffsetMs, Handler eventHandler, EventListener eventListener) {
        this(manifestFetcher, (MediaPresentationDescription) manifestFetcher.getManifest(), trackSelector, dataSource, adaptiveFormatEvaluator, new SystemClock(), liveEdgeLatencyMs * 1000, elapsedRealtimeOffsetMs * 1000, true, eventHandler, eventListener);
    }
    ...
    public DashChunkSource(ManifestFetcher<MediaPresentationDescription> manifestFetcher, DashTrackSelector trackSelector, DataSource dataSource, FormatEvaluator adaptiveFormatEvaluator, long liveEdgeLatencyMs, long elapsedRealtimeOffsetMs, boolean startAtLiveEdge, Handler eventHandler, EventListener eventListener) {
        this(manifestFetcher, (MediaPresentationDescription) manifestFetcher.getManifest(), trackSelector, dataSource, adaptiveFormatEvaluator, new SystemClock(), liveEdgeLatencyMs * 1000, elapsedRealtimeOffsetMs * 1000, startAtLiveEdge, eventHandler, eventListener);
    }
        ...
        if (selectedFormat == null) {
            ...
            if (queue.isEmpty()) {
                ...
                if (this.live && nextSegmentStartTimeUs < this.availableRangeValues[0]) {
                    ...
                    if (!this.currentManifest.dynamic) {
                        ...
                        if (previous.parentId == lastPeriodHolder.localIndex && ((RepresentationHolder) lastPeriodHolder.representationHolders.get(previous.format.id)).isLastSegment(previous.chunkIndex)) {
                    ...
                    if (periodHolder == null) {
                        ...
                        startingNewPeriod = true;
                    } else if (!periodHolder.isIndexUnbounded() && ((RepresentationHolder) periodHolder.representationHolders.get(previous.format.id)).isLastSegment(previous.chunkIndex)) {
            ...
            if (pendingInitializationUri == null && pendingIndexUri == null) {
                ...
                if (queue.isEmpty()) {
                    ...
                    segmentNum = ((MediaChunk) queue.get(out.queueSize - 1)).chunkIndex + 1;
                }
                Chunk nextMediaChunk = newMediaChunk(periodHolder, representationHolder, this.dataSource, mediaFormat, segmentNum, this.evaluation.trigger);
    ...
    private static String getMediaMimeType(Format format) {
        ...
        if (MimeTypes.APPLICATION_MP4.equals(formatMimeType) && "stpp".equals(format.codecs)) {
    ...
    private Chunk newMediaChunk(PeriodHolder periodHolder, RepresentationHolder representationHolder, DataSource dataSource, MediaFormat mediaFormat, int segmentNum, int trigger) {
        ...
        if (mimeTypeIsRawText(format.mimeType)) {
            return new SingleSampleMediaChunk(dataSource, dataSpec, 1, format, startTimeUs, endTimeUs, segmentNum, this.enabledTrack.trackFormat, null, periodHolder.localIndex);
        }
        return new ContainerMediaChunk(dataSource, dataSpec, trigger, format, startTimeUs, endTimeUs, segmentNum, sampleOffsetUs, representationHolder.extractorWrapper, mediaFormat, this.enabledTrack.adaptiveMaxWidth, this.enabledTrack.adaptiveMaxHeight, periodHolder.drmInitData, mediaFormat != null, periodHolder.localIndex);
    }
    ...
    private TimeRange getAvailableRange(long nowUnixTimeUs) {
        ...
        return new DynamicTimeRange(firstPeriod.getAvailableStartTimeUs(), lastPeriod.isIndexUnbounded() ? MediaFormat.OFFSET_SAMPLE_RELATIVE : lastPeriod.getAvailableEndTimeUs(), (this.systemClock.elapsedRealtime() * 1000) - (nowUnixTimeUs - (this.currentManifest.availabilityStartTime * 1000)), this.currentManifest.timeShiftBufferDepth == -1 ? -1 : this.currentManifest.timeShiftBufferDepth * 1000, this.systemClock);
    }
    ...
    private void notifyAvailableRangeChanged(final TimeRange seekRange) {
        if (this.eventHandler != null && this.eventListener != null) {
            this.eventHandler.post(new Runnable() {
                public void run() {
                    DashChunkSource.this.eventListener.onAvailableRangeChanged(seekRange);
                }
            });
        }
    }
