...
public class Zip4jUtil {
    ...
    public static void setFileHidden(File file) throws ZipException {
        if (file == null) {
            ...
            try {
                Runtime.getRuntime().exec(new StringBuffer("attrib +H \"").append(file.getAbsolutePath()).append("\"").toString());
            } catch (IOException e) {
            }
        }
    }
    ...
    public static void setFileArchive(File file) throws ZipException {
        if (file == null) {
            ...
            try {
                if (file.isDirectory()) {
                    Runtime.getRuntime().exec(new StringBuffer("attrib +A \"").append(file.getAbsolutePath()).append("\"").toString());
                } else {
                    Runtime.getRuntime().exec(new StringBuffer("attrib +A \"").append(file.getAbsolutePath()).append("\"").toString());
                }
            } catch (IOException e) {
    ...
    public static void setFileSystemMode(File file) throws ZipException {
        if (file == null) {
            ...
            try {
                Runtime.getRuntime().exec(new StringBuffer("attrib +S \"").append(file.getAbsolutePath()).append("\"").toString());
            } catch (IOException e) {
            }
        }
    }
            ...
            if (!isStringNotNullAndNotEmpty(currZipFile)) {
                ...
                for (int i = 0; i <= numberOfThisDisk; i++) {
                    if (i == numberOfThisDisk) {
                        ...
                        retList.add(zipFileName.indexOf(".") >= 0 ? currZipFile.substring(0, currZipFile.lastIndexOf(".")) : new StringBuffer(String.valueOf(currZipFile)).append(fileExt).append(i + 1).toString());
                    }
                }
                return retList;
            } else {
